...
class UNetModel(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        self.model_channels = model_channels = config['model_channels']
        self.out_channels = out_channels = config['out_channels']
        self.num_res_blocks = num_res_blocks = config['num_res_blocks']
        self.attention_resolutions = attention_resolutions = config['attention_resolutions']
        self.dropout = dropout = config['dropout']
        self.channel_mult = channel_mult = config['channel_mult']
        self.conv_resample = conv_resample = config['conv_resample']
    ...
    def inner_dtype(self):
        ...
        """
        # print(next(self.input_blocks.parameters()).dtype)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        h = h.type(x.dtype)
        output = self.out(h)
        model_output, model_var_values = th.split(output, C, dim=1)
        return model_output
